...
def calculate_dist(posA: tuple, posB: tuple) -> float:
...
def calculate_bullet_time(ship: dict, aster: dict) -> float:
    ...
    asteroid_ship_x = ship_pos_x - aster["aster"]["position"][0]
    asteroid_ship_y = ship_pos_y - aster["aster"]["position"][1]
    
    asteroid_ship_theta = math.atan2(asteroid_ship_y,asteroid_ship_x)
    
    asteroid_direction = math.atan2(aster["aster"]["velocity"][1], aster["aster"]["velocity"][0]) # Velocity is a 2-element array [vx,vy].
    my_theta2 = asteroid_ship_theta - asteroid_direction
    cos_my_theta2 = math.cos(my_theta2)
    # Need the speeds of the asteroid and bullet. speed * time is distance to the intercept point
    asteroid_vel = math.sqrt(aster["aster"]["velocity"][0]**2 + aster["aster"]["velocity"][1]**2)
    
    # Determinant of the quadratic formula b^2-4ac
    targ_det = (-2 * aster["dist"] * asteroid_vel * cos_my_theta2)**2 - (4*(asteroid_vel**2 - bullet_speed**2) * (aster["dist"]**2))
    
    # Combine the Law of Cosines with the quadratic formula for solve for intercept time. Remember, there are two values produced.
    intrcpt1 = ((2 * aster["dist"] * asteroid_vel * cos_my_theta2) + math.sqrt(targ_det)) / (2 * (asteroid_vel**2 -bullet_speed**2))
    intrcpt2 = ((2 * aster["dist"] * asteroid_vel * cos_my_theta2) - math.sqrt(targ_det)) / (2 * (asteroid_vel**2-bullet_speed**2))
...
def calculate_shot_angle(ship:dict, aster:dict) -> float:
    bullet_t = calculate_bullet_time(ship, aster)
    ship_pos_x, ship_pos_y = ship["position"]
    # Calculate the intercept point. The work backwards to find the ship's firing angle my_theta1.
    # Velocities are in m/sec, so bullet_t is in seconds. Add one tik, hardcoded to 1/30 sec.
    intrcpt_x = aster["aster"]["position"][0] + aster["aster"]["velocity"][0] * (bullet_t+1/30)
    intrcpt_y = aster["aster"]["position"][1] + aster["aster"]["velocity"][1] * (bullet_t+1/30)

    
    my_theta1 = math.atan2((intrcpt_y - ship_pos_y),(intrcpt_x - ship_pos_x))
    return my_theta1

def calculate_shot_angle_rel_to_ship_heading(ship: dict, aster:dict) -> float:
    my_theta1 = calculate_shot_angle(ship, aster)
    
    # Lastly, find the difference betwwen firing angle and the ship's current orientation. BUT THE SHIP HEADING IS IN DEGREES.
    shooting_theta = my_theta1 - ((math.pi/180)*ship["heading"])
...
class TeamController(KesslerController):
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool]:
        ...
        # Wrap all angles to (-pi, pi)
        shooting_theta = (shooting_theta + math.pi) % (2 * math.pi) - math.pi
        shooting_theta = calculate_shot_angle_rel_to_ship_heading(ship_state, closest_asteroid)
